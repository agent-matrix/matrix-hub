...
def sync_registry_gateways_new2(db: Session) -> None:
    """
    Re-affirm registration of all mcp_server entities in the MCP-Gateway.
    """
    from .gateway_client import register_server

    # Retrieve all federated server entries from the DB
    servers = db.query(Entity).filter(Entity.type == "mcp_server").all()
    for ent in servers:
        # The manifest's mcp_registration field should have the 'server' spec
        mcp_reg = getattr(ent, 'mcp_registration', None) or {}
        server_spec = mcp_reg.get('server') if isinstance(mcp_reg, dict) else None
        if not isinstance(server_spec, dict):
            continue

        # Normalize URL: append /sse for SSE transport if not present
        url = server_spec.get('url', '').rstrip('/')
        transport = server_spec.get('transport', '').upper()
        if transport == 'SSE' and not url.endswith('/sse'):
            url += '/sse'

        payload = {
            'name': server_spec.get('name', ent.name),
            'description': server_spec.get('description', ''),
            'url': url,
            'associated_tools': server_spec.get('associated_tools', []),
            'associated_resources': server_spec.get('associated_resources', []),
            'associated_prompts': server_spec.get('associated_prompts', []),
        }
        try:
            register_server(payload, idempotent=True)
            log.info("✓ Synced gateway: %s", payload['name'])
...
def sync_registry_gateways_new(db: Session) -> None:
    """
    Re-affirm registration of all mcp_server entities in MCP-Gateway.
    This can be called from a CLI or API endpoint to resync after a rebuild.
    """
    from .gateway_client import register_server
    # Query all MCP server entities from the DB
    servers = db.query(Entity).filter(Entity.type == "mcp_server").all()
    for ent in servers:
        # Extract the server registration spec from the manifest field
        mcp_reg = ent.mcp_registration.get("server") if hasattr(ent, 'mcp_registration') else None
        if not isinstance(mcp_reg, dict):
            continue
        payload = {
            "name": mcp_reg.get("name", ent.name),
            "description": mcp_reg.get("description", ""),
            "url": mcp_reg.get("url", "").rstrip("/") + ("/sse" if mcp_reg.get("transport","").upper()=="SSE" else ""),
            "associated_tools": mcp_reg.get("associated_tools", []),
            "associated_resources": mcp_reg.get("associated_resources", []),
            "associated_prompts": mcp_reg.get("associated_prompts", []),
        }
        try:
            register_server(payload, idempotent=True)
            log.info("✓ Synced gateway in MCP-Gateway: %s", payload["name"])
        except Exception as e:
            log.warning("⚠️ Failed to sync gateway %s: %s", payload["name"], e)


def install_entity_old(
    db: Session,
    entity_id: str,
