...
# Cloudflare TLS: ENABLE_CF_TLS=1 (default) to serve HTTPS with origin cert.
#
...
# ---------------------------
# Cloudflare Origin TLS (ADDITIVE; enabled by default)
# ---------------------------
ENABLE_CF_TLS="${ENABLE_CF_TLS:-1}"               # default ON for production
CERTS_DIR_HOST="${CERTS_DIR_HOST:-/etc/ssl/matrixhub}"
CERTS_DIR_CONT="/etc/ssl/matrixhub"
CF_CERT_FILE="${CF_CERT_FILE:-cf-origin.pem}"   # override if you use custom names
CF_KEY_FILE="${CF_KEY_FILE:-cf-origin.key}"
...
stop_rm_if_exists() {
  ...
  if [ -n "$(docker ps -aq -f name="^${name}$")" ]; then
    step "Removing ${name}"
    docker rm "${name}" >/dev/null 2>&1 || docker rm "${name}" >/dev/null
  fi
}
...
# If Cloudflare TLS is enabled, require the cert and key
if [[ "${ENABLE_CF_TLS}" == "1" ]]; then
  [[ -d "${CERTS_DIR_HOST}" ]] || die "Certs dir not found: ${CERTS_DIR_HOST}"
  [[ -f "${CERTS_DIR_HOST}/${CF_CERT_FILE}" ]] || die "Missing cert file: ${CERTS_DIR_HOST}/${CF_CERT_FILE}"
  [[ -f "${CERTS_DIR_HOST}/${CF_KEY_FILE}"  ]] || die "Missing key file:  ${CERTS_DIR_HOST}/${CF_KEY_FILE}"
fi
...
# - Run gateway in the background via start script.
# - Default to ONE worker to avoid concurrent startup/migrations on SQLite.
# - If DATABASE_URL is clearly non-sqlite, allow WEB_CONCURRENCY to raise it.
HUB_CMD=$'  set -e\n'
HUB_CMD+=$'  # Kick off Gateway in background using the provided start script.\n'
HUB_CMD+='  bash '"${CONT_START_SCRIPT}"$' &\n'
HUB_CMD+=$'\n'
HUB_CMD+=$'  # Source /app/.env to detect DATABASE_URL reliably.\n'
HUB_CMD+=$'  set -a; [ -f /app/.env ] && . /app/.env || true; set +a\n'
HUB_CMD+=$'\n'
HUB_CMD+=$'  WORKERS=1\n'
HUB_CMD+=$'  if [[ -n "${DATABASE_URL:-}" ]] && [[ ! "${DATABASE_URL}" =~ ^sqlite ]]; then\n'
HUB_CMD+=$'    WORKERS="${WEB_CONCURRENCY:-2}"\n'
HUB_CMD+=$'  else\n'
HUB_CMD+=$'    echo "ℹ Using SQLite or no DATABASE_URL set → forcing --workers 1"\n'
HUB_CMD+=$'  fi\n'

if [[ "${ENABLE_CF_TLS}" == "1" ]]; then
  HUB_CMD+=$'  exec /app/.venv/bin/gunicorn src.app:app \\\n'
  HUB_CMD+=$'    -k uvicorn.workers.UvicornWorker \\\n'
  HUB_CMD+=$'    --workers "${WORKERS}" \\\n'
  HUB_CMD+=$'    --bind 0.0.0.0:443 \\\n'
  HUB_CMD+='    --certfile '"${CERTS_DIR_CONT}/${CF_CERT_FILE}"$' \\\n'
  HUB_CMD+='    --keyfile  '"${CERTS_DIR_CONT}/${CF_KEY_FILE}"$'\n'
else
  HUB_CMD+=$'  exec /app/.venv/bin/gunicorn src.app:app \\\n'
  HUB_CMD+=$'    -k uvicorn.workers.UvicornWorker \\\n'
  HUB_CMD+=$'    --workers "${WORKERS}" \\\n'
...
# - Mount Gateway env to /app/.env.gateway.*
# - Mount the working start script
# - Optionally mount Cloudflare origin certs (read-only)
docker run -d \
  --name "${CONTAINER_NAME}" \
  --network "${NETWORK_NAME}" \
  -p "${HUB_PORT}:443" \
  ...
  $( [[ "${ENABLE_CF_TLS}" == "1" ]] && echo -v "${CERTS_DIR_HOST}:${CERTS_DIR_CONT}:ro" ) \
  --entrypoint /bin/bash \
  "${FULL_IMAGE_NAME}" \
  -lc "${HUB_CMD}"
...
if [[ "${ENABLE_CF_TLS}" == "1" ]]; then
  info "Hub:       https://localhost:${HUB_PORT}/"
  info "Gateway:   http://localhost:${GATEWAY_PORT}/"
  info "Cloudflare TLS: enabled (serving HTTPS with origin cert)"
  info "Ensure DNS is proxied (orange cloud) and SSL/TLS mode is 'Full (strict)'."
else
  info "Hub:       http://localhost:${HUB_PORT}/"
  info "Gateway:   http://localhost:${GATEWAY_PORT}/"
  warn "Cloudflare TLS disabled (ENABLE_CF_TLS=0). For production, enable it."
fi
